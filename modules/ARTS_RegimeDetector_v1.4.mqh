...
private:
   double CalculateLinearRegressionRSquared()
   {
      ...
      // FIX v1.4: Start at i=1 to exclude current bar (non-repainting)
      for(int i = 1; i <= m_lookback_linreg; i++)
      {
         double x = i - 1; // Adjust x to start from 0
   ...
   double CalculateATRPercentile()
   {
      ...
      // FIX v1.4: Use bar 1 for current ATR (non-repainting)
      ...
      if(CopyBuffer(atr_handle, 0, 1, m_lookback_atr_percentile, historical_atr) <= 0)
      {
         IndicatorRelease(atr_handle);
         return 50.0;
      }
