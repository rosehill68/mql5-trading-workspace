...
public:
   ...
   // FIX v1.4: Added shift parameter but defaults to bar 1 for non-repainting
   double GetATR(int shift = 1)
   {
      if(!m_initialized) return 0;
      // FIX v1.4: Ensure we use bar 1 (confirmed closed bar) by default
      if(shift < 1) shift = 1;
      if(CopyBuffer(m_handle_atr, 0, shift, 1, m_buffer_atr) <= 0) return 0;
      return m_buffer_atr[0];
   }
   
   double GetEMA20(int shift = 1)
   {
      if(!m_initialized) return 0;
      // FIX v1.4: Ensure we use bar 1 (confirmed closed bar) by default
      if(shift < 1) shift = 1;
   ...
   // FIX v1.4: Uses bar 1 for Donchian calculation (non-repainting)
   double GetDonchianUpper(int shift = 1)
   {
      if(!m_initialized) return 0;
      // FIX v1.4: Start from bar 1 to ensure non-repainting
      if(shift < 1) shift = 1;
   ...
   double GetDonchianLower(int shift = 1)
   {
      if(!m_initialized) return 0;
      // FIX v1.4: Start from bar 1 to ensure non-repainting
      if(shift < 1) shift = 1;
   ...
   double GetKeltnerUpper(double multiplier = 2.0, int shift = 1)
   {
   ...
   double GetKeltnerLower(double multiplier = 2.0, int shift = 1)
   {
      double ema = GetEMA20(shift);
      double atr = GetATR(shift);
      if(ema == 0 || atr == 0) return 0;
   ...
   double GetKeltnerMiddle(int shift = 1)
   {
